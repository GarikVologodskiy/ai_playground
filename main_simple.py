...
Главная цель: amount = сколько людей будет жить.

Правила:
1. Считай только людей: взрослых, детей, ребенка, семью, пару, компанию.
2. НЕ считай даты: 11.07, 23.07, 10.08, 20.08, 30.07.
3. НЕ считай возраст детей: 6 лет, 4 года, 3,5 г., 10 и 3 года.
4. НЕ считай количество дней: на 5-7 дней, на 20 дней.
5. НЕ считай номера и места как людей, если отдельно указаны люди:
   "3-местный номер" — это номер, не 3 человека.
   "2 двухспалки" — это кровати/комнаты, не люди.
   "3 местный и 2 местный эконом" — это тип номеров, не люди.
6. Если написано "2 взрослых и 3 ребенка" — amount = 5.
7. Если написано "муж, жена и ребенок" — amount = 3.
8. Если написано "3 взрослых и 3 детей" — amount = 6.
9. Если написано "семья 4 человека" — amount = 4.
10. Если написано "два взрослых, два ребенка" — amount = 4.
11. Если написано "нас 3 взрослых, 2 детей" — amount = 5.
12. Если в тексте указано только "3-местный номер" и нет людей — amount = 3.
13. Если совсем невозможно понять количество людей — amount = 1.

Примеры:
Текст: "2 взрослых и 3 ребенка (6,6 и 4 года)"
Ответ: {"amount": 5}

Текст: "Муж, жена и 3,5 г. ребенок"
Ответ: {"amount": 3}

Текст: "3-местный и 2-местный эконом. 3 взрослых и 3 детей"
Ответ: {"amount": 6}

Текст: "Интересует жилье 3 местный номер"
Ответ: {"amount": 3}

Текст: "Семья 4 человека, 2 взрослых, дети 10 и 3 года"
Ответ: {"amount": 4}

Текст: "два взрослых, два ребенка"
Ответ: {"amount": 4}

Текст: "Нас 3 взрослых, 2 детей 5 лет и 3,9 лет"
Ответ: {"amount": 5}
"""
